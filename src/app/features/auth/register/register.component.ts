...
export class RegisterComponent {
  hide = true;          // toggle for password
  hideConfirm = true;   // toggle for confirm password
  ...
  submit() {
    ...
    if (this.f.value.password !== this.f.value.confirmPassword) {
    ...
    this.auth.register(this.f.value).subscribe({
      ...
      error: (e) => {
        console.error(e);
        this.snack.open(e?.error?.message || 'Registration failed', 'ok');
      }
    });
  }
}
